...
The CPU, or Central Processing Unit, is one of the most important parts of any PC. As the primary hub, it processes the instructions that come from programs, the operating system, or other components in the computer. The CPU is a small electronic chip that is made up of billions of transistors.

Transistors, when being used to create a CPU, act as on/off switches, transmitting electrical signals as binary code sequences(machine language). This allows the standardization of a protocol called the Instruction Cycle. Once a program instruction is made, the CPU, using machine code, is able to take (fetch), process (decode), and execute instructions repeatedly until the entire program instruction is complete.
...
Below is a table showing common CPUs, their socket type, and available chipsets.
...
| 4GB | 8GB | 16GB | 32GB+ |
|-----|-----|------|------|
|Bare Minimum. Adequate only for word processing, e-mail, and web browsing | Standard. Minimum required for gaming.| Ideal for gaming and most systems and tasks | Any kind of system intensive, workstation-grade tasks; programming, virtual machines, photo and video editing and compositing. |
